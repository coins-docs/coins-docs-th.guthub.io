...
data.status	| The order status. Possible values are: `PROCESSING`, `SUCCESS`, and `FAILED`. `PROCESSING` means the server is processing the order. `SUCCESS` means the order has been completed successfully. `FAILED` means the order processing failed. Please check the `errorMessage` field in the Order List API response for the detailed failure reason.
